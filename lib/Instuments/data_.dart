//использование sqflite, увидел в одном гайде. Получилось похоже на работу с апи в мгушной прилажке
...
class DatabaseUsage {
  ...
  Future<void> insertTodo(Check todo) async {
    Database _db = await database();
  ...
  Future<List<Check>> getTodo(int taskId) async {
    Database _db = await database();
    List<Map<String, dynamic>> todoMap = await _db.rawQuery("SELECT * FROM todo WHERE taskId = $taskId");
    return List.generate(todoMap.length, (index) {
      return Check(id: todoMap[index]['id'], title: todoMap[index]['title'], taskId: todoMap[index]['taskId'], isDone: todoMap[index]['isDone']);
    });
  }
